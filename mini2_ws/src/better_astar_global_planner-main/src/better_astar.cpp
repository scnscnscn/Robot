...
namespace BAstar_planner
{
                                  ...
                                  std::vector<geometry_msgs::PoseStamped> &plan)
  {
    ...
    if (isStartAndGoalCellsValid(startCell, goalCell))
    {
      ...
      if (bestPath.size() > 0)
      {
        ...
        // 调用 optimizePath 函数进行路径优化
        plan = optimizePath(plan, 0.5); // 1.5 为安全距离（米）
  ...
  {    
    ...
    d_Manhatten = abs(x1-x2)+abs(y1-y2);         //Manhatten Distance
    //distance = sqrt(pow(x1,x2)+pow(y1,y2));       //Euclidean Distance
    d_Diagnol = min(abs(x1-x2),abs(y1-y2))  *sqrt(2)  + max(abs(x1-x2),abs(y1-y2)) - min(abs(x1-x2),abs(y1-y2));   //Diagonal Distance
    distance = 0.4*d_Manhatten+0.6*d_Diagnol;

    w = distance/(abs(x1-x2),abs(y1-y2));
    h = w *w* distance;
...
vector<geometry_msgs::PoseStamped> BAstarPlannerROS::optimizePath(const vector<geometry_msgs::PoseStamped>& path, float safety_distance)
{
    vector<geometry_msgs::PoseStamped> optimized_path;
    int path_size = path.size();

    if (path_size < 2) {
        return path; // 如果路径节点少于2个，直接返回原路径
    }
    ...
    for (int i = 1; i < path_size - 1; i++)
    {
        ...
        // 检查是否共线，若共线则跳过中间节点
        while (next_i < path_size && arePointsCollinear(path[i - 1], path[i], path[next_i]))
        {
            i = next_i;
            next_i++;
        }
        ...
        // 检查路径段是否安全
        while (next_i < path_size && isPathSafe(path[i - 1], path[next_i], safety_distance))
        {
            i = next_i;
            next_i++;
        }
    ...
    // 添加终点到优化后的路径中
    optimized_path.push_back(path[path_size - 1]);

    return optimized_path;
}

bool BAstarPlannerROS::arePointsCollinear(const geometry_msgs::PoseStamped& p1, const geometry_msgs::PoseStamped& p2, const geometry_msgs::PoseStamped& p3)
{
    float dx1 = p2.pose.position.x - p1.pose.position.x;
    float dy1 = p2.pose.position.y - p1.pose.position.y;
    float dx2 = p3.pose.position.x - p2.pose.position.x;
    float dy2 = p3.pose.position.y - p2.pose.position.y;

    return fabs(dx1 * dy2 - dy1 * dx2) < 1e-6; // 如果行列式接近0，表示共线
}

bool BAstarPlannerROS::isPathSafe(const geometry_msgs::PoseStamped& p1, const geometry_msgs::PoseStamped& p2, float safety_distance)
{
    // 计算两点之间的距离
    float distance = hypot(p2.pose.position.x - p1.pose.position.x, p2.pose.position.y - p1.pose.position.y);
    
    // 检查路径是否安全：这里简化为直接返回true，如果需要实际判断障碍物，可以添加具体实现
