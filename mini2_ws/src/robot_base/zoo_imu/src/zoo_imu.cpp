...
ZooIMU::ZooIMU(ros::NodeHandle nh, ros::NodeHandle pnh):
  ...
  is_calibrated_(false)
{
  ...
  raw_sub_ = nh_.subscribe("raw_imu", 5, &ZooIMU::rawCallback, this);

  imu_cal_srv_ = nh_.advertiseService("imu/calibrate_imu", &ZooIMU::calibrateCallback, this);
  ...
  if (use_accelerometer_ || use_gyroscope_)
  {
    ...
    ZooIMU::fillRowMajor(linear_acc_covar_, linear_acc_stdev_);

    pnh_.param<double>("imu/angular_vel_stdev", angular_vel_stdev_, 0.0);
    ZooIMU::fillRowMajor(angular_vel_covar_, angular_vel_stdev_);
  }
  ...
  if (use_magnetometer_)
  {
    ...
    if (use_mag_msg_)
    {
      ...
      ZooIMU::fillRowMajor(magnetic_field_covar_, magnetic_field_stdev_);
    }
    else
    {
...
void ZooIMU::rawCallback(const zoo_imu::RawImuConstPtr& raw_msg)
{
...
bool ZooIMU::calibrateCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
...
void ZooIMU::fillRowMajor(boost::array<double, 9> & covar, double stdev)
{
