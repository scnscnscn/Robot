...
class ZooIMU
{
  private:
    ...
    const double MILIGAUSS_TO_TESLA_SCALE = 0.0000001;  // From Milligauss [mG] to Tesla [T]
    ...
    void rawCallback(const zoo_imu::RawImuConstPtr& raw_msg);
  ...
  public:
    ZooIMU(ros::NodeHandle nh, ros::NodeHandle pnh);
    virtual ~ZooIMU(void){};
};
