...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    else if(dataString.compare("开始导航。") == 0)
    {   
		std_msgs::String msg;
	  	std::stringstream ss;
	 	ss<<"开始导航";
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 6010cacd, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
    ...
    ros::NodeHandle n;
    ros::Subscriber sub =n.subscribe("/robot_voice/nlu_topic", 1000,voiceWordsCallback);
	nav_pub = n.advertise<std_msgs::String>("/robot_voice/nav_topic", 1000);
    ros::spin();
